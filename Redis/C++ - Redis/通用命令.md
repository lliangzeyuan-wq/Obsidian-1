...
| `EXISTS`  | 判断一个或多个 key 是否存在           | `redis.exists("key1");`<br><br>`redis.exists({"key1", "key2"});`                                                            | (integer) , 值为这里面存在的key的个数 |     |     |     |
| `DEL`     | 删除一个或多个 key                | `redis.del("key1");`<br><br>`redis.del({"key1", "key2", "key3"});`                                                          | (integer) , 值为这里面存在的key的个数 |     |     |     |
| `KEYS`    | 按通配符模糊查询 key               | `std::vector<std::string> keys;`<br><br>`redis.keys("*name*", std::back_inserter(keys));`                                   |                            |     |     |     |
| `EXPIRE`  | 为 key 设置过期时间（秒）            | `redis.expire("mykey", std::chrono::seconds(10));`                                                                          | 1（成功），0（失败）                |     |     |     |
| `TTL`     | 查询 key 的剩余过期时间（秒）          | `redis.ttl("mykey");`                                                                                                       | 剩余的时间（秒）                   |     |     |     |
| `PTTL`    | 查询 key 的剩余过期时间（毫秒）         | `redis.pttl("mykey");`                                                                                                      | 剩余的时间（毫秒）                  |     |     |     |
...
| `LPUSH`   | 向 List 类型 key 左侧插入元素       | `redis.lpush("key2", "value");`                                                                                             | long long , 表示做ch          |     |     |     |
